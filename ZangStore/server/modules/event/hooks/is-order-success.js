const ns = '[event.hooks.is-order-success]';
...
const processEvent = function(context, next) {
  ...
  U.P()
    .then(async() => {
      logger.info(fn, 'check if order processing completed', context.processStatus);
      if (isOrderProcessingCompleted(context.processStatus)) {
        try { // update the order as success
          await OrderBackend.setOrderStatus(order, ORDER_STATUS_SUCCESS, context);
        } catch (err1) {
