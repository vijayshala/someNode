const ns = '[ip-office][event-hooks][validate-ipoffice-order]';
  ...
  U.P()
    ...
    .then(() => {
      if (!payload.company || !payload.company.industry) {
        ...
        errors.push(new ErrorMessage(fn, VIOLATIONS.FIELD_VALUE_INVALID, {
          text: 'Thank you for choosing IPO Cloud as your solution, unfortunately we\'re not HIPAA compliant yet, so we can\'t provide you our service because you\'re in healthcare.  We\'ll be sure to let know when we get our certification, and we hope can have you back then!  Feel free to reach out to <a href="mailto:customersuccess@zang.io">customersuccess@zang.io</a> if you have any questions',
    ...
    .then(() => {
      ...
      let number = null, e911Address = null;
      for (let item of payload.items) {
        const itemContext = findCartItemContext(item);
        const tags = itemContext && itemContext.tags;
        if (tags && tags.indexOf(tagToFind) > -1) {
          number = itemContext.helper && itemContext.helper.number;
          e911Address = itemContext.helper && itemContext.helper.e911Address;
        }
      }
      if (!e911Address) {
        errors.push(new ErrorMessage(fn, VIOLATIONS.FIELD_IS_EMPTY, {
          text: 'E911 address is required.',
          resource: ['E911_REQUIRED'],
        }, 'items.attribute[did-main].helper.e911Address'));
      }
      if (!number) {
        errors.push(new ErrorMessage(fn, VIOLATIONS.FIELD_IS_EMPTY, {
