const ns = '[zang-spaces][event-hooks][validate-zang-spaces-order]';
...
const processEvent = function(context, next) {
  ...
  U.P()
    .then(() => {
      ...
      if (!payload.company || !payload.company.industry) {
        errors.push(new ErrorMessage(fn, VIOLATIONS.FIELD_IS_EMPTY, {
          text: 'Company industry is required',
          resource: ['VIOLATION.FIELD_IS_EMPTY', 'COMPANY_INDUSTRY'],
        }, 'company.industry'));
      } else if (!isAcceptableIndustry(payload)) {
        errors.push(new ErrorMessage(fn, VIOLATIONS.FIELD_VALUE_INVALID, {
          text: 'Thank you for choosing Avaya Spaces as your solution, unfortunately we\'re not HIPAA compliant yet, so we can\'t provide you our service because you\'re in healthcare.  We\'ll be sure to let know when we get our certification, and we hope can have you back then!  Feel free to reach out to <a href="mailto:customersuccess@zang.io">customersuccess@zang.io</a> if you have any questions',
          resource: ['HEALTHCARE_DENY_MESSAGE'],
        }, 'company.industry'));
      }
    })
    .then(async() => {
      ...
      if (Array.isArray(exists) && exists.length > 0) {
        ...
        errors.push(new ErrorMessage(fn, VIOLATIONS.FIELD_VALUE_INVALID, {
          text: 'It seems that you have already purchased Avaya Spaces for this company. Please select a different company.',
          resource: ['IT_SEEMS_THAT_YOU_HAVE_ALREADY_PURCHASED_SPACES_FOR_THIS_COMPANY_THE_ORDER_CANNOT_BE_PLACED'],
        }, 'company.nid'));
      }
    })
    .then(async() => {
    ...
    .then(() => {
      ...
      if (totalQuantity == 0) {
        errors.push(new ErrorMessage(fn, VIOLATIONS.FIELD_VALUE_INVALID, {
          text: 'Each Avaya Spaces plan requires at least one user type. Please go back to the devices selection step and change your configuration.',
          resource: ['EACH_SPACES_PLAN_REQUIRES_AT_LEAST_ONE_STANDARD_OR_POWER_USER_PLEASE_GO_BACK_TO_THE_DEVICES_SELECTION_STEP_AND_CHANGE_YOUR_CONFIGURATION'],
        }, 'items.quantity'));
      }
    })
    // .then(async() => {
