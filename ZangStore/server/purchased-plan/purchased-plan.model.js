...
const PurchasedPlanItemSchemaDef = new Schema({
  level: {
    type: Number,
    default: 0,
  },
...
PurchasedPlanItemSchemaDef.set('toJSON', {
  getters: true,
  virtuals: true
});

PurchasedPlanItemSchemaDef.set('toObject', {
  getters: true,
  virtuals: true
});

const PurchasedPlanSchemaDef = new Schema({
  orderIds: [{ // the original order ID which creates the purchased-plan
    type: Schema.Types.ObjectId,
    ref: ORDER_TABLE_NAME
  }], // amendment orders list
  status: {
    type: String,
    enum: PURCHASED_PLAN_STATUSES,
  },
  ...
  subscriptions: [{
    ...
    status: {
      type: String,
      enum: PURCHASED_PLAN_SUBSCRIPTION_STATUSES,
    },
...
PurchasedPlanSchemaDef.set('toJSON', {
  getters: true,
  virtuals: true
});
PurchasedPlanSchemaDef.set('toObject', {
  getters: true,
  virtuals: true
});

PurchasedPlanSchemaDef.index({ orderIds: 1 });
PurchasedPlanSchemaDef.index({ status: 1, 'company.nid': 1 });

const PurchasedPlanSchema = mongoose.model(PURCHASED_PLAN_TABLE_NAME, PurchasedPlanSchemaDef);

module.exports = { PurchasedPlanSchema };
